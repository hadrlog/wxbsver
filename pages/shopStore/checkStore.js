...
Page({
    ...
    api.Cpic_fetchPost(api.KCCX, {
      ...
      if (err == "00") {
        ...
        if (rdata == 0) {
          ...
          var tempArr = { name: good_detail.product_name, gnum: e, price: cprice, amount: good_detail.store_count, samount: good_detail.store_count };
  ...
  markSureAction: function () {
    ...
    for (var i = 0; i < gd.length; i++) {
      ...
      var pcode = temp.gnum;
      var num = temp.amount;
      var cost = temp.samount;
      var dic = { "product_code": pcode, "sys_store": num, "actual_store": cost, "remark": "" };
      outArr.push(dic);
    }
    api.Cpic_fetchPost(api.TOTALCACULATE, { "bussiness_date": ctime, "hander_user": handle_user, "shop_no": shopnum, "remark": '', 'check_data': outArr }, (err, res) => {
      console.log("err:" + err + "DIC:" + res.ReturnMsg + 'GROUP：' + res.data);
      var taxt = '';
      if (err == "00") {
        taxt = '成功';
